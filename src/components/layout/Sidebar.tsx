import { LandPlot, ShieldCheck, Sprout } from "lucide-react";
import { NavLink } from "react-router-dom";
const link = ({ isActive }: { isActive: boolean }) =>
  `flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${isActive ? "bg-brand-50 text-brand-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`;
  ...
  return (
    <aside className='w-64 shrink-0 border-l border-gray-200 bg-white h-screen sticky top-0 flex flex-col'>
      ...
      <nav className='flex-1 px-3 py-4 flex flex-col gap-1'>
        <p className='px-3 pb-1.5 text-xs font-medium text-gray-400'>القائمة</p>
        <NavLink
          to='/'
          end
          className={link}>
